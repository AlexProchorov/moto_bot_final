...
from utils.weather import get_current_weather, get_forecast, clothing_recommendation, get_weather_by_coords
...
async def send_weather_to_users(bot: Bot, is_morning: bool):
    """Отправляет персонализированную погоду в ЛС каждому зарегистрированному пользователю с указанным районом."""
    ...
    for user in users:
        ...
        if not coords:
            logger.warning(f"Нет координат для района {user['district']}")
            continue
        ...
        if is_morning:
            text = (
                ...
                f"Прогноз погоды в вашем районе ({user['district']}) на сегодня:\n"
            ...
            text = (
                ...
                f"Погода в вашем районе ({user['district']}) сейчас:\n"
