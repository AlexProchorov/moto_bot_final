...
def rules_message() -> str:
    return (
        "🏍 *Добро пожаловать в мотосообщество!*\n\n"
        "Перед началом общения просим ознакомиться с короткими правилами.\n\n"
        "❌ *Здесь не приветствуются:*\n"
        "• конфликты\n"
        "• переход на личности и оскорблядство\n"
        "• доказывание своей правоты до талого\n\n"
        "✅ *Мы вас любим за:*\n"
        "• подколы\n"
        "• активное участие в чате и выездах\n"
        "• взаимопомощь и поддержку\n\n"
        "🙌 *Оставайся с нами – будет весело и надёжно!*"
    )

def ride_rules_message() -> str:
    return (
        "🏍 *Правила покатушек:*\n\n"
        "• Безопасная езда превыше всего! Соблюдаем ПДД и дистанцию.\n"
        "• Всегда есть 1 ведущий и 1 закрывающий в хвосте. Не обгоняйте ведущего и не отставайте от закрывающего.\n"
        "• Если хочешь выебываться (показать навыки) — предупреди, мы оценим, но только безопасно для всех.\n"
        "• Ведущий может меняться по ходу маршрута, сообщи, если хочешь предложить дорогу или возглавить.\n"
        "• Взаимопомощь и уважение друг к другу — залог отличной поездки!\n\n"
        "Приятных и безопасных километров! 🛣️"
    )

def welcome_with_rules() -> str:
    return (
        "🏍 *Добро пожаловать в мотосообщество!*\n\n"
        "Перед началом общения просим ознакомиться с короткими правилами.\n\n"
        "❌ *Здесь не приветствуются:*\n"
        ...
        "• активное участие в чате и выездах\n"
        "• взаимопомощь и поддержку\n\n"
        "🙌 *Будь с нами – будет весело и надёжно!*"
    )